...
Regions = df.TrdNodDensLocNameAdm1.unique()
...
# Create a pop-up window and select region or market data
pfont = ("Arial", 18)
layout1 = [ 
            [sg.Text('Select Region or Market analysis',font = ("Arial", 18)),
             sg.Listbox(['Region','Market'],select_mode=sg.LISTBOX_SELECT_MODE_SINGLE,size=(20,2),font = pfont)],
            [sg.Button('Ok',font = pfont), sg.Button('Cancel',font = pfont)]
         ]
# Create the Window
window = sg.Window('Region or Market', layout1)
...
reg_mkt =  values[0][0]#w.result

if reg_mkt == 'Region':

    layout2 = [ 
            [sg.Text('Select Region: ',font = ("Arial", 18)),
             sg.Listbox(sorted(Regions),select_mode=sg.LISTBOX_SELECT_MODE_SINGLE,size=(20,len(Regions)),font = pfont)],
            [sg.Button('Ok',font = pfont), sg.Button('Cancel',font = pfont)]
        ]
    
    # Create the Window
    window = sg.Window('Select the Region', layout2)
    ...
    mfi_region =  values[0][0]#w.result
    df_mfi = df.loc[df['TrdNodDensLocNameAdm1'] == mfi_region]
    mfi_title = values[0][0]

else:
    layout2 = [ 
            [sg.Text('Select Market: ',font = ("Arial", 18)),
             sg.Listbox(sorted(Markets),select_mode=sg.LISTBOX_SELECT_MODE_SINGLE,size=(20,len(Markets)),font = pfont)],
            [sg.Button('Ok',font = pfont), sg.Button('Cancel',font = pfont)]
        ]
    
    # Create the Window
    window = sg.Window('Select the Market', layout2)
    ...
    # Retrieve the desired market and fiter the results to create a new dataframe
    mfi_markets =  values[0][0]#w.result
    df_mfi = df.loc[df['MarketName'] == mfi_markets]
    mfi_title = values[0][0]
...
fig = px.line_polar(plotdf, r = 'value', theta = 'variable', line_close = True, 
                    title="Region: "+mfi_title+" ( MFI = {mfi_t:.1f} )".format(mfi_t=MFI))
